...
window.renderBoqKeywordBox = function(item) {
  ...
  return `
    <div class="boq-verified-box" style="background: linear-gradient(135deg, #f0fdf4 0%, #dcfce7 100%); border: 2.5px solid #86efac; border-left: 8px solid #16a34a; border-radius: 8px; padding: 12px 18px; margin: 12px 0; color: #14532d; display: flex; align-items: center; gap: 12px; box-shadow: 0 3px 8px rgba(22, 163, 74, 0.15);">
      <span style="font-size: 1.8rem; line-height: 1;">🎯</span>
      <div style="display: flex; align-items: center; flex-wrap: wrap; gap: 8px;">
        <span style="font-size: 1.15rem; font-weight: 800; color: #15803d; white-space: nowrap;">ตรวจพบ Keyword =</span>
        <span style="background: #16a34a; color: #ffffff; padding: 3px 14px; border-radius: 6px; font-size: 1.35rem; font-weight: 900; box-shadow: 0 2px 5px rgba(22, 163, 74, 0.3); letter-spacing: 0.3px; white-space: nowrap;">${keyword}</span>
      </div>
    </div>
  `;
};
...
window.PROVINCE_TO_REGION = {
  // 1. กทม
  'กรุงเทพมหานคร': { id: 'bkk', name: 'กทม.', icon: '🏙️', bg: '#e0f2fe', color: '#0369a1', border: '#7dd3fc' },
  // 2. ภาคกลาง
  'นนทบุรี': { id: 'central', name: 'ภาคกลาง', icon: '🏛️', bg: '#f1f5f9', color: '#334155', border: '#cbd5e1' },
  'ปทุมธานี': { id: 'central', name: 'ภาคกลาง', icon: '🏛️', bg: '#f1f5f9', color: '#334155', border: '#cbd5e1' },
  'พระนครศรีอยุธยา': { id: 'central', name: 'ภาคกลาง', icon: '🏛️', bg: '#f1f5f9', color: '#334155', border: '#cbd5e1' },
  'สระบุรี': { id: 'central', name: 'ภาคกลาง', icon: '🏛️', bg: '#f1f5f9', color: '#334155', border: '#cbd5e1' },
  'นครนายก': { id: 'central', name: 'ภาคกลาง', icon: '🏛️', bg: '#f1f5f9', color: '#334155', border: '#cbd5e1' },
  'ลพบุรี': { id: 'central', name: 'ภาคกลาง', icon: '🏛️', bg: '#f1f5f9', color: '#334155', border: '#cbd5e1' },
  'สิงห์บุรี': { id: 'central', name: 'ภาคกลาง', icon: '🏛️', bg: '#f1f5f9', color: '#334155', border: '#cbd5e1' },
  'อ่างทอง': { id: 'central', name: 'ภาคกลาง', icon: '🏛️', bg: '#f1f5f9', color: '#334155', border: '#cbd5e1' },
  'ชัยนาท': { id: 'central', name: 'ภาคกลาง', icon: '🏛️', bg: '#f1f5f9', color: '#334155', border: '#cbd5e1' },
  'สมุทรปราการ': { id: 'central', name: 'ภาคกลาง', icon: '🏛️', bg: '#f1f5f9', color: '#334155', border: '#cbd5e1' },
  'สมุทรสงคราม': { id: 'central', name: 'ภาคกลาง', icon: '🏛️', bg: '#f1f5f9', color: '#334155', border: '#cbd5e1' },
  'สมุทรสาคร': { id: 'central', name: 'ภาคกลาง', icon: '🏛️', bg: '#f1f5f9', color: '#334155', border: '#cbd5e1' },
  'นครปฐม': { id: 'central', name: 'ภาคกลาง', icon: '🏛️', bg: '#f1f5f9', color: '#334155', border: '#cbd5e1' },
  'สุพรรณบุรี': { id: 'central', name: 'ภาคกลาง', icon: '🏛️', bg: '#f1f5f9', color: '#334155', border: '#cbd5e1' },
  // 3. ภาคตะวันออก
...
// Default keywords - Firetrade Engineering PCL (FTE) 5 Product Groups (v1.0)
...
// Exclude list to reject false positives
...
function getKeywords() {
  ...
  if (ver !== '1.0') {
    localStorage.setItem('fte_custom_keywords', JSON.stringify(DEFAULT_KEYWORDS));
...
window.renderKeywords = function() {
  ...
  for (const [group, list] of Object.entries(kws)) {
    ...
    html += `
      <div style="background: #ffffff; padding: 8px 12px; border-radius: 6px; border: 1px solid #e2e8f0;">
        <div style="font-weight: bold; font-size: 0.88rem; color: #dc2626; margin-bottom: 6px;">${groupName} (${list.length} คำ):</div>
        <div>
  ...
  if (badge) badge.textContent = `${totalCount} คำหลัก (5 หมวดสินค้า FTE)`;
};
...
window.hideProject = function(id, btn) {
  ...
  const hidden = JSON.parse(localStorage.getItem('fte_hidden_projects') || '[]');
  if (!hidden.includes(id)) {
    hidden.push(id);
    localStorage.setItem('fte_hidden_projects', JSON.stringify(hidden));
  }
    ...
    banner.innerHTML = `
      <div class="status-banner-content">
        ...
        <div>
          ...
          <span style="color: #166534; font-size: 0.82rem; margin-left: 6px;">(บันทึกลงระบบ AI แล้ว • ช่วยจดจำสเปกที่ FTE รับทำ)</span>
        </div>
      </div>
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (toggleHideRejected) {
    const savedToggle = localStorage.getItem('fte_auto_hide_rejected');
    if (savedToggle !== null) {
    ...
    toggleHideRejected.addEventListener('change', (e) => {
      localStorage.setItem('fte_auto_hide_rejected', e.target.checked);
      if (e.target.checked) {
        document.body.classList.add('hide-rejected-active');
      } else {
  ...
  try {
    ...
    let html = items.map((item, idx) => {
      ...
      const coreFteKeywords = [
        'ดับเพลิง', 'แจ้งเหตุเพลิงไหม้', 'เตือนอัคคีภัย', 'fire alarm', 'สปริงเกอร์', 'สปริงเกลอร์',
        'fire sprinkler', 'ปั๊มดับเพลิง', 'เครื่องสูบน้ำดับเพลิง', 'fire pump', 'สารสะอาด',
        'novec', 'fm-200', 'fm200', 'co2 ดับเพลิง', 'ตู้ดับเพลิง', 'สายส่งน้ำดับเพลิง', 'fire hose',
        'หัวรับน้ำดับเพลิง', 'ไฟฉุกเฉิน', 'ป้ายทางหนีไฟ', 'max bright', 'ถังดับเพลิง'
      ];
      const isTopFit = coreFteKeywords.some(k => titleLower.includes(k)) && 
                       (titleLower.includes('ซื้อ') || titleLower.includes('จ้าง') || titleLower.includes('ติดตั้ง') || titleLower.includes('ปรับปรุง') || titleLower.includes('ซ่อมแซม'));

      // 2. Disguised Package Fit (Comprehensive Building MEP / Hospital / Data Center with Fire Package):
      // The project title is framed as broad civil/MEP/hospital but inside the BOQ (ปร.4/บก.01) there is a full fire protection/alarm system
      const disguisedKeywords = [
        'ระบบประกอบอาคาร', 'งานระบบสุขาภิบาลและดับเพลิง', 'งานระบบวิศวกรรม', 'ปรับปรุงอาคาร', 'ก่อสร้างอาคาร', 'ศูนย์การแพทย์', 'โรงพยาบาล', 'อาคารผู้ป่วย'
      ];
      ...
      if (!matchedKw && item.project_name) {
        const t = item.project_name;
        if (t.includes('แจ้งเหตุเพลิงไหม้') || t.includes('fire alarm') || t.includes('เตือนอัคคีภัย')) matchedKw = 'ระบบแจ้งเหตุเพลิงไหม้';
        else if (t.includes('สปริงเกอร์') || t.includes('สปริงเกลอร์') || t.includes('sprinkler')) matchedKw = 'สปริงเกอร์ดับเพลิง';
        else if (t.includes('เครื่องสูบน้ำดับเพลิง') || t.includes('ปั๊มดับเพลิง') || t.includes('fire pump')) matchedKw = 'เครื่องสูบน้ำดับเพลิง';
        else if (t.includes('สารสะอาด') || t.includes('novec') || t.includes('fm-200')) matchedKw = 'ก๊าซสารสะอาดดับเพลิง';
        else if (t.includes('ตู้สายฉีดน้ำ') || t.includes('ตู้ดับเพลิง') || t.includes('สายส่งน้ำ')) matchedKw = 'ตู้สายส่งน้ำดับเพลิง';
        else if (t.includes('ไฟฉุกเฉิน') || t.includes('ป้ายทางหนีไฟ')) matchedKw = 'ไฟฉุกเฉิน/ป้ายทางหนีไฟ';
        else if (t.includes('ถังดับเพลิง') || t.includes('เครื่องดับเพลิง')) matchedKw = 'เครื่องดับเพลิงยกหิ้ว';
      }
        ...
        container.innerHTML = `
          <div style="background: #f8fafc; border: 1.5px dashed #cbd5e1; border-radius: 12px; padding: 40px 20px; text-align: center; margin: 20px 0;">
            ...
            <p style="color: #64748b; font-size: 0.95rem; max-width: 520px; margin: 0 auto; line-height: 1.5;">
              ระบบ AI In-Memory Scanner สแกนไฟล์ PDF และ ปร.4 อย่างต่อเนื่องทุกวัน หากมีประกาศใหม่ที่พบสเปกของ FTE จะแสดงขึ้นที่นี่ทันที
            </p>
          </div>
        `;
      }
