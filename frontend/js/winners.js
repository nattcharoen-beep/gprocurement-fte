...
async function loadWinnerStats() {
  try {
    ...
    tbody.innerHTML = topCompanies.map((s, rankIdx) => {
      const isFTE = (s.winner_name || '').includes('ไฟร์เทรด') || (s.winner_name || '').toUpperCase().includes('FTE');
      const rowBg = isFTE ? 'background: #fefce8; border-left: 4px solid #eab308;' : '';
      const nameTag = isFTE 
...
async function loadWinnerList(append = false) {
  ...
  try {
    ...
    const hiddenIds = JSON.parse(localStorage.getItem('fte_hidden_projects') || '[]');
    const items = (res.data || []).filter(it => {
      const pid = it.project_id || it.id || '';
      if (hiddenIds.includes(pid)) return false;
      const isFTE = (it.winner_name || '').includes('ไฟร์เทรด') || (it.winner_name || '').toUpperCase().includes('FTE');
      // Never exclude FTE's own won projects!
      if (!isFTE && typeof isExcluded === 'function' && isExcluded(it.project_name)) return false;
      return true;
    });
    ...
    let html = items.map((item, idx) => {
      ...
      const isFTE = (item.winner_name || '').includes('ไฟร์เทรด') || (item.winner_name || '').toUpperCase().includes('FTE');
      const cardBorder = isFTE 
        ? 'border-left: 6px solid #eab308; background: #fffdf5; box-shadow: 0 4px 14px rgba(234, 179, 8, 0.15);' 
        : 'border-left: 4px solid var(--badge-w0, #0066cc);';
      const fteBadge = isFTE 
        ? `<span style="background: #fef08a; color: #854d0e; font-weight: 800; font-size: 0.85rem; padding: 2px 10px; border-radius: 6px; border: 1px solid #facc15; display: inline-flex; align-items: center; gap: 4px; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">⭐ ผลงาน FTE (บริษัทเรา)</span>` 
        : '';

      const winnerBoxHtml = isFTE
...
function filterFTEOnly(btn) {
  ...
  if (input) {
    if (winnerSearchKeyword === 'ไฟร์เทรด') {
      input.value = '';
      winnerSearchKeyword = '';
      if (btn) {
      ...
      input.value = 'ไฟร์เทรด';
      winnerSearchKeyword = 'ไฟร์เทรด';
      if (btn) {
