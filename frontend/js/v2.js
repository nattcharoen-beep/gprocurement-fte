// frontend/js/v2.js — FTE B2G Command Center V2.0 Controller
...
if (typeof window.groupLabels === 'undefined') {
  window.groupLabels = {
    'fire_alarm': 'ระบบแจ้งเหตุเพลิงไหม้',
    'fire_sprinkler_pump': 'สปริงเกอร์และปั๊มดับเพลิง',
    'fire_suppression_gas': 'สารสะอาดและก๊าซดับเพลิง',
    'fire_hydrant_equipment': 'ตู้สายฉีดน้ำและวาล์ว',
...
let v2SelectedGroups = ['fire_alarm', 'fire_sprinkler_pump', 'fire_suppression_gas', 'fire_hydrant_equipment', 'safety_ppe_emergency'];
let v2SearchKeyword = '';
...
const v2FeedbackKey = 'fte_user_feedback';
const v2RejectTallyKey = 'fte_reject_tally';
...
// ============================================================================
// Firetrade Engineering (FTE) Master Keywords
// ============================================================================
const V2_DEFAULT_KEYWORDS = {
  fire_alarm: [
    "ระบบแจ้งเหตุเพลิงไหม้", "แจ้งเหตุเพลิงไหม้", "ระบบเตือนอัคคีภัย", "เตือนอัคคีภัย",
    "Fire Alarm", "Fire Alarm System", "Fire Alarm Control Panel", "FACP",
    "ตู้ควบคุมระบบแจ้งเหตุเพลิงไหม้", "อุปกรณ์ตรวจจับควัน", "Smoke Detector",
    "อุปกรณ์ตรวจจับความร้อน", "Heat Detector", "Beam Detector", "Aspirated Smoke Detector", "VESDA",
    "Manual Pull Station", "กระดิ่งเตือนภัย", "Fire Alarm Bell", "ไซเรน", "Horn Strobe", "Strobe Light",
    "Notifier", "Morley", "System Sensor", "GST"
  ],
  fire_sprinkler_pump: [
    "หัวกระจายน้ำดับเพลิง", "สปริงเกอร์ดับเพลิง", "สปริงเกลอร์ดับเพลิง", "ระบบสปริงเกอร์ดับเพลิง",
    "Fire Sprinkler", "Sprinkler System", "หัวสปริงเกอร์", "ESFR", "Dry Type Sprinkler",
    "Deluge Valve", "Alarm Check Valve", "Preaction Valve", "Preaction System",
    "เครื่องสูบน้ำดับเพลิง", "ระบบเครื่องสูบน้ำดับเพลิง", "Fire Pump", "Fire Pump System",
    "ปั๊มดับเพลิง", "ระบบปั๊มดับเพลิง", "Diesel Fire Pump", "Electric Fire Pump",
    "Jockey Pump", "ปั๊มรักษาแรงดัน", "ตู้ควบคุมเครื่องสูบน้ำดับเพลิง", "Fire Pump Controller", "NFPA 20",
    "Patterson", "Clarke", "Tornatech", "Viking", "Tyco"
  ],
  fire_suppression_gas: [
    "ระบบสารสะอาดดับเพลิง", "ระบบดับเพลิงด้วยสารสะอาด", "สารสะอาดดับเพลิง", "Clean Agent",
    "Novec 1230", "Novec", "FK-5-1-12", "FM-200", "FM200", "HFC-227ea", "Inert Gas", "IG-541", "IG-100",
    "ระบบก๊าซคาร์บอนไดออกไซด์", "ระบบ CO2 ดับเพลิง", "CO2 Fire Suppression",
    "ระบบดับเพลิงห้องครัว", "Kitchen Fire Suppression", "Kitchen Hood System", "Wet Chemical",
    "ระบบละอองน้ำดับเพลิง", "Water Mist System", "ระบบโฟมดับเพลิง", "Foam System", "น้ำยาโฟมดับเพลิง", "AFFF",
    "ถังดับเพลิง", "เครื่องดับเพลิงยกหิ้ว", "เครื่องดับเพลิง", "ถังดับเพลิงเคมีแห้ง", "ถังดับเพลิง CO2", "ถังดับเพลิงสารสะอาด",
    "Badger", "Total Fire", "Ansul", "Kidde"
  ],
  fire_hydrant_equipment: [
    "ตู้สายฉีดน้ำดับเพลิง", "ตู้ดับเพลิง", "Fire Hose Cabinet", "FHC", "ตู้โฮสริว", "ตู้โฮสแร็ค",
    "สายส่งน้ำดับเพลิง", "สายฉีดน้ำดับเพลิง", "Fire Hose", "Fire Hose Reel", "กงล้อสายส่งน้ำดับเพลิง",
    "หัวฉีดน้ำดับเพลิง", "หัวรับน้ำดับเพลิง", "Siamese Connection", "หัวดับเพลิง", "Fire Hydrant",
    "วาล์วระบบดับเพลิง", "OS&Y Gate Valve", "Butterfly Valve ดับเพลิง", "Check Valve ดับเพลิง",
    "ข้อต่อกรูฟ", "Grooved Coupling", "Grooved Fitting", "Mechanical Tee", "ท่อดับเพลิง", "ระบบท่อดับเพลิง",
    "NIBCO", "Dixon", "Potter Roemer", "SRI", "Victaulic"
  ],
  safety_ppe_emergency: [
    "ไฟฉุกเฉิน", "โคมไฟฉุกเฉิน", "Emergency Light", "ไฟฉุกเฉินอัตโนมัติ",
    "ป้ายทางออกฉุกเฉิน", "ป้ายไฟทางออกฉุกเฉิน", "ป้ายทางหนีไฟ", "ป้ายไฟทางหนีไฟ", "Exit Sign", "Max Bright",
    "สายไฟทนไฟ", "Fire Resistant Cable", "FRC Cable", "FRC", "MCI-DRAKA",
    "ตู้เก็บสารเคมี", "ตู้เก็บสารไวไฟ", "Flammable Cabinet", "Safety Cabinet",
    "อ่างล้างตาฉุกเฉิน", "ฝักบัวฉุกเฉิน", "Emergency Shower", "Emergency Eyewash",
    "ชุดดับเพลิง", "ชุดผจญเพลิง", "Fire Fighting Suit", "เครื่องช่วยหายใจ", "SCBA"
  ]
};

function getV2Keywords() {
  const ver = localStorage.getItem('fte_kw_version');
  if (ver !== '1.0') {
    localStorage.setItem('fte_custom_keywords', JSON.stringify(V2_DEFAULT_KEYWORDS));
    localStorage.setItem('fte_kw_version', '1.0');
    return JSON.parse(JSON.stringify(V2_DEFAULT_KEYWORDS));
  }
  const saved = localStorage.getItem('fte_custom_keywords');
  if (saved) {
...
function saveV2Keywords(kws) {
  localStorage.setItem('fte_custom_keywords', JSON.stringify(kws));
}
  ...
  try {
    if (typeof api !== 'undefined' && api.getMe) {
      ...
      if (user) {
        ...
        if (userNameEl) userNameEl.textContent = user.name || user.email || 'FTE แอดมิน';
        if (userRoleEl) userRoleEl.textContent = user.role === 'admin' ? 'ผู้ดูแลระบบ (Admin)' : 'ทีมงาน Firetrade (FTE)';
        if (userAvatarEl) {
...
window.switchV2View = function(viewName) {
  ...
  const viewMeta = {
    overview: { title: 'ภาพรวมระบบ', desc: 'ศูนย์บัญชาการจัดซื้อจัดจ้างภาครัฐ สรุปโอกาสงานและมูลค่าที่เฝ้าติดตามสำหรับ บมจ. ไฟร์เทรดเอ็นจิเนียริ่ง' },
    opportunities: { title: 'โอกาสงานค้นพบใหม่', desc: 'รายการประกาศเชิญชวน, ร่าง TOR, และราคากลางระบบดับเพลิงที่ตรงกับคีย์เวิร์ด' },
    boq: { title: 'สแกนพบในไฟล์ BOQ', desc: 'โครงการที่มุดสแกนพบรายการสเปกระบบดับเพลิง/แบรนด์ของ FTE ในไฟล์ PDF / ปร.4' },
    ...
    competitor: { title: 'ข่าวกรองคู่แข่ง', desc: 'วิเคราะห์บริษัทที่ชนะประมูลระบบดับเพลิงบ่อยที่สุด และส่วนลดราคากลางเฉลี่ย' },
    keywords: { title: 'จัดการคีย์เวิร์ด', desc: 'เฝ้าระวังคำหลักระบบดับเพลิงใน e-GP และเพิ่มคีย์เวิร์ดเฉพาะของ FTE' },
    profile: { title: 'โปรไฟล์ Firetrade (FTE)', desc: 'หนังสือรับรองดิจิทัล ทุนจดทะเบียน และเกณฑ์คุณสมบัติความพร้อมยื่นงานรัฐ' }
  };
...
function extractProvince(item) {
  ...
  if (p && p !== 'ไม่ระบุ' && p !== 'null') {
    return p.replace(/^จ(ังหวัด|\.)\s*/, '').trim();
  }
  const text = `${item.project_name || ''} ${item.department || ''}`;
  const m = text.match(/จังหวัด([ก-๙]+)/);
  if (m && m[1]) {
    return m[1].replace(/นคราชสีมา/, 'นครราชสีมา');
  }
  if (window.PROVINCE_TO_REGION) {
    for (const provName of Object.keys(window.PROVINCE_TO_REGION)) {
      if (text.includes(provName)) {
        return provName;
      }
    }
  }
...
function updateV2ZoneCounters(list, statsData) {
  ...
  // Update Overview Product Groups (FTE 5 Core Groups)
  const groupCounts = {
    fire_alarm: 0,
    fire_sprinkler_pump: 0,
    fire_suppression_gas: 0,
    fire_hydrant_equipment: 0,
    safety_ppe_emergency: 0
  };
  list.forEach(item => {
  ...
  setTxt('ov-gcount-alarm', `${groupCounts.fire_alarm} งาน`);
  setTxt('ov-gcount-sprinkler', `${groupCounts.fire_sprinkler_pump} งาน`);
  setTxt('ov-gcount-gas', `${groupCounts.fire_suppression_gas} งาน`);
  setTxt('ov-gcount-hydrant', `${groupCounts.fire_hydrant_equipment} งาน`);
  setTxt('ov-gcount-safety', `${groupCounts.safety_ppe_emergency} งาน`);
}
...
function generateV2CardHTML(item, index) {
  ...
  // Smart Province Extraction (Prefer explicit title province name if available)
  let displayProv = extractProvince(item);
  let provBadge = '';
  if (displayProv && displayProv !== 'ไม่ระบุ' && displayProv !== 'null') {
    let formattedProv = displayProv;
    if (!formattedProv.startsWith('จังหวัด') && !formattedProv.startsWith('จ.') && formattedProv !== 'กรุงเทพมหานคร') {
      formattedProv = `จ.${formattedProv}`;
    }
    provBadge = `<span style="background: #e0f2fe; color: #0284c7; font-weight: 700; font-size: 0.84rem; padding: 2px 10px; border-radius: 6px; border: 1px solid #bae6fd;">📍 ${formattedProv}</span>`;
  }
      ...
      <div class="card-layer-feedback" style="background: #f8fafc; border-radius: 8px; padding: 12px 14px; border: 1px solid #e2e8f0;">
        <div class="feedback-question-row" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 8px;">
          <div style="display: flex; align-items: center; gap: 6px;">
            ...
            <strong style="color: #334155; font-size: 0.92rem;">งานนี้ตรงกับ FTE ไหม?</strong>
          </div>
...
function generateV2TableRowHTML(item, index) {
  ...
  const groupText = window.groupLabels[item.product_group] || item.product_group;
  const prov = extractProvince(item);
  const provText = (prov && prov !== 'ไม่ระบุ' && prov !== 'null') ? (prov.startsWith('จ.') ? prov : `จ.${prov}`) : '';
  ...
  return `
    <tr class="${origin?.isPdf ? 'table-row-pdf' : ''}" id="trow-${item.id}" onclick="openV2Inspector('${item.id}')" style="cursor: pointer;">
      ...
      <td>
        ...
        ${provText ? `<div style="color: #0284c7; font-size: 0.78rem; font-weight: 600; margin-top: 2px;">📍 ${provText}</div>` : ''}
      </td>
      <td class="col-budget">
        ${window.formatMoney(item.budget)}
      </td>
...
let currentV2ViewMode = localStorage.getItem('fte_v2_view_mode') || 'card';

window.setV2ViewMode = function(mode) {
  currentV2ViewMode = mode;
  try { localStorage.setItem('fte_v2_view_mode', mode); } catch(e) {}
...
function loadSavedProfile() {
  const profileKey = 'fte_company_profile';
  let profile = {
...
window.saveCompanyProfile = function(event) {
  ...
  const profile = {
    ...
    maxBudget: Number(document.getElementById('v2-prof-max-budget').value) || 100000000,
    expertise: document.getElementById('v2-prof-expertise').value,
    certs: document.getElementById('v2-prof-certs').value
  };

  localStorage.setItem('fte_company_profile', JSON.stringify(profile));
  loadSavedProfile();
  showToast('💾 บันทึกโปรไฟล์บริษัท FTE เรียบร้อย');
};
    ...
    contentEl.innerHTML = `
      <div>
        <div style="display: flex; gap: 6px; align-items: center; margin-bottom: 8px; flex-wrap: wrap;">
          ...
          ${provText ? `<span style="font-size: 0.76rem; font-weight: 600; color: #0284c7; background: #e0f2fe; padding: 2px 7px; border-radius: 4px;">📍 ${provText}</span>` : ''}
        </div>
          ...
          <div class="v2-boq-terminal">
            ...
            ` : `
              <div style="color: #94a3b8; font-size: 0.82rem;">
                พบคำค้นสำคัญ <mark>${origin.keyword}</mark> ในชื่อโครงการตรงตามสเปกของ FTE 100%
              </div>
            `}
          </div>
        </div>
