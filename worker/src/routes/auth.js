...
router.post('/register', async (c) => {
  ...
  return c.json({ 
    data: { 
      message: 'สมัครสมาชิกสำเร็จแล้ว! ระบบได้ส่งแจ้งเตือนไปยังผู้ดูแลระบบเรียบร้อยแล้ว กรุณารอการอนุมัติก่อนเข้าใช้งาน' 
    } 
  });
});
...
router.post('/login', async (c) => {
  ...
  if (!user && (identifier === 'admin' || identifier === 'fte-admin')) {
    user = await db.prepare("SELECT * FROM users WHERE role = 'admin' ORDER BY id ASC LIMIT 1").first();
  }
  ...
  if (user.status === 'pending') {
    return c.json({ error: 'บัญชีของคุณอยู่ระหว่างรอผู้ดูแลระบบอนุมัติการใช้งาน' }, 403);
  }
  ...
  // Secure admin fallback using RECOVERY_KEY secret to prevent lockouts
  if (!isValid && user.role === 'admin' && env.RECOVERY_KEY) {
    const pTrim = (password || '').trim();
    if (pTrim && pTrim === env.RECOVERY_KEY.trim()) {
      isValid = true;
      try {
