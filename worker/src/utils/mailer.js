...
export async function sendAdminNotification(env, newUser) {
  ...
  const adminEmail = (env.ADMIN_EMAIL || env.GMAIL_RECIPIENTS || gmailUser).trim();

  if (!gmailUser || !gmailPass || !adminEmail) {
    console.warn('[Mailer] Missing GMAIL_USER, GMAIL_APP_PASSWORD or ADMIN_EMAIL in worker env');
    return false;
  }

  const approveUrl = (env.FRONTEND_URL || 'https://gprocurement-fte.pages.dev') + '/admin.html';
...
<body style="font-family: Arial, sans-serif; background-color: #f8fafc; padding: 20px; color: #1e293b;">
  <div style="max-width: 560px; margin: 0 auto; background: #ffffff; border-radius: 8px; border: 1px solid #e2e8f0; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1);">
    <div style="background: #dc2626; color: #ffffff; padding: 20px; text-align: center;">
      <h2 style="margin: 0; font-size: 1.25rem;">FTE • Firetrade | GProcurement Finder</h2>
      <p style="margin: 5px 0 0 0; color: #fecaca; font-size: 0.9rem;">แจ้งเตือน: มีสมาชิกใหม่สมัครเข้าใช้งาน (รอการอนุมัติ)</p>
    </div>
    <div style="padding: 24px;">
      ...
      <div style="background: #f1f5f9; border-radius: 6px; padding: 16px; margin: 18px 0; border: 1px solid #cbd5e1;">
        <div style="margin-bottom: 8px;"><strong>👤 ชื่อผู้ใช้ (Username):</strong> <span style="color: #dc2626; font-weight: 700;">${newUser.username || '-'}</span></div>
      ...
      <p style="font-size: 0.85rem; color: #64748b; text-align: center; margin-top: 20px;">
        หรือไปที่ URL: <a href="${approveUrl}" style="color: #dc2626;">${approveUrl}</a>
      </p>
    </div>
    <div style="background: #f8fafc; border-top: 1px solid #e2e8f0; padding: 12px; text-align: center; font-size: 0.75rem; color: #94a3b8;">
      ระบบแจ้งเตือนอัตโนมัติ • บริษัท ไฟร์เทรดเอ็นจิเนียริ่ง จำกัด (มหาชน) (Firetrade Engineering PCL)
    </div>
  </div>
</body>
</html>
