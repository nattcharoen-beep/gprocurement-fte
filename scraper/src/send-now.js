...
function getEnvVars() {
  const env = {
    ...
    GMAIL_APP_PASSWORD: '',
    GMAIL_RECIPIENTS: '',
    D1_API_URL: 'https://gprocurement-fte.natt-charoen.workers.dev'
  };
...
async function promptPassword(userEmail) {
  ...
  return new Promise(resolve => {
    ...
    console.log(`2. เข้าสู่ระบบด้วยบัญชีผู้ส่ง: ${userEmail || '(บัญชี Gmail ที่กำหนดใน .env)'}`);
    console.log('3. ตั้งชื่อแอปว่า "FTE Procurement Finder" แล้วกดสร้าง (Create)');
...
async function main() {
  console.log('=============================================================');
  console.log('🚀 FTE GPROCUREMENT FINDER - ส่งอีเมลสรุปงานทันที');
  console.log('=============================================================');

  let env = getEnvVars();
  let user = (env.GMAIL_USER || '').trim();
  let pass = (env.GMAIL_APP_PASSWORD || '').replace(/\s+/g, '');
  let recipient = (env.GMAIL_RECIPIENTS || '').trim();

  if (!user) {
    console.error('❌ ยังไม่ได้ระบุ GMAIL_USER ในไฟล์ .env');
    console.log('💡 กรุณาเปิดไฟล์ .env และกรอก GMAIL_USER=your_email@gmail.com');
    process.exit(1);
  }

  if (!recipient) {
    recipient = user;
    console.log(`ℹ️ ไม่ได้ระบุ GMAIL_RECIPIENTS จะส่งไปยังอีเมลผู้ส่ง: ${user}`);
  }

  if (!pass || pass.length < 10) {
    pass = await promptPassword(user);
    if (pass && pass.length >= 10) {
  ...
  const subject = `🔥 FTE จัดซื้อจัดจ้างภาครัฐ — สรุปงานประจำวัน ${dateStr}`;
  ...
  for (const rec of recipients) {
    ...
    try {
      const info = await transporter.sendMail({
        from: `"FTE • จัดซื้อจัดจ้างภาครัฐ" <${user}>`,
        to: rec,
        subject: subject,
        html: html
      });
