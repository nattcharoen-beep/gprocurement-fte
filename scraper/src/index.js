/**
 * Firetrade Engineering PCL (FTE) - GProcurement Finder Orchestrator
 * Runs the Precision & Lifecycle 2-Pass Harvester for e-GP v5.
 * Delivers automated daily fire protection intelligence digest to subscribers.
 */
...
async function main() {
  loadEnvIfAvailable();
  log('Starting FTE GProcurement Intelligence Harvester (Firetrade Engineering PCL)...');

  // 1. Load config from env vars
  const D1_API_URL = process.env.D1_API_URL || 'https://gprocurement-fte.workers.dev';
  const D1_API_KEY = process.env.D1_API_KEY;
  const envRecipients = process.env.GMAIL_RECIPIENTS || process.env.ADMIN_EMAIL || '';
